...
func tarStreamRewrite(archivePath string, mods map[string]func(*tar.Header, io.Reader) (*tar.Header, []byte, error)) error {
	...
	if err != nil {
		return fmt.Errorf("unable to access archive: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("unable to read archive magic bytes")
	}
	...
	if err != nil {
		...
		return fmt.Errorf("unable to create compressor")
	}
	...
	for {
		...
		if modifier, ok := mods[header.Name]; ok {
			...
			if newHeader != nil {
				...
				if err := tarWriter.WriteHeader(newHeader); err != nil {
					...
					outFile.Close()
					os.Remove(outputPath)
					return fmt.Errorf("unable to write header for %s: %w", header.Name, err)
				}
				if _, err := tarWriter.Write(data); err != nil {
					...
					outFile.Close()
					os.Remove(outputPath)
					return fmt.Errorf("unable to write data for %s: %w", header.Name, err)
				}
			}
		} else {
			...
			if _, err := io.Copy(tarWriter, tarReader); err != nil {
				...
				outFile.Close()
				os.Remove(outputPath)
				return fmt.Errorf("unable to copy data for %s: %w", header.Name, err)
			}
		}
	}
	...
	if err := os.Chmod(outputPath, archiveInfo.Mode().Perm()); err != nil {
		return fmt.Errorf("unable to set output permissions: %w", err)
	}

	// Replace the modified archive with the original one
	if err := os.Rename(outputPath, archivePath); err != nil {
		os.Remove(outputPath)
		return fmt.Errorf("archive remapped, but unable to rename temp file as original: %w", err)
	}
