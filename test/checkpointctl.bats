...
@test "Run checkpointctl show with tar file with valid config.dump and valid spec.dump and network.status" {
	cp data/config.dump "$TEST_TMP_DIR1"
	cp data/spec.dump "$TEST_TMP_DIR1"
...
@test "Run checkpointctl inspect with tar file with valid config.dump and valid spec.dump and network.status" {
	...
	[ "$status" -eq 0 ]
	[[ ${lines[6]} == *"Podman"* ]]
	[[ ${lines[7]} == *"IP: 10.88.0.9"* ]]
	[[ ${lines[8]} == *"MAC: f2:99:8d:fb:5a:57"* ]]
}
